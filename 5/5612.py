...
with webdriver.Chrome() as browser:
    ...
    for cookie in cookies:
        ...
        age = 0 if not age else int(age.split()[1])
        skills = len(browser.find_element(By.ID, 'skillsList').find_elements(By.CSS_SELECTOR, 'li'))
        hackers.append([cookie['value'], age, skills])
        browser.delete_cookie(cookie['name'])

    print(max(hackers, key=lambda a: (-a[1], a[2]))[0])
